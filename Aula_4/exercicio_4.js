...
let semGorjeta = 0;
for (let vendas of receita) {
    semGorjeta = semGorjeta + vendas.valor
}
console.log("Total de vendas sem gorjeta foi: R$",semGorjeta)

//Exercício 2: Qual o valor total de vendas incluindo a gorjeta?
let valorTotal = 0
const receitaTotal = (valor, gorjeta) => {
    let somaTotal = valor + gorjeta
    return somaTotal
}

for (let vendasT of receita) {
    valorTotal = valorTotal + receitaTotal (vendasT.valor, vendasT.gorjeta)
}
console.log("Valor total incluindo as gorjetas: R$",valorTotal)

//Exercício 3: Quantas vendas foram abaixo de 50 reais?
let qnt = 0
for (let vendasB of receita) {
    if (vendasB.valor < 50) {
        qnt++
    }
}
console.log("Vendas abaixo de R$50,00, foram",qnt)